...
from models import CustomResnet101_centercam_1net_action
...
model = CustomResnet101_centercam_1net_action(num_classes=4)
...
for epoch in range(epochs):
    ...
    model.train()
    running_loss = 0.0
    for inputs,actions, labels in train_loader:
    ...
    with torch.no_grad():
        for inputs,actions, labels in valid_loader:
            inputs = inputs.to(device)
            labels = labels.to(device)
            actions = actions.to(device)
            outputs = model(inputs,actions)
...
total = 0
with torch.no_grad():
    for inputs, actions,labels in test_loader:
        inputs = inputs.to(device)
        labels = labels.to(device)
        actions = actions.to(device)
        outputs = model(inputs,actions)
